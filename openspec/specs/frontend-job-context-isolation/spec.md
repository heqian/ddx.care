...
The client SHALL persist the minimum job ID, token, and expiry metadata needed to recover a pending or completed job within the same browser session. Expired, cancelled, reset, or explicitly cleared contexts SHALL be removed.
