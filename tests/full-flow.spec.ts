...
test.describe("Full diagnosis flow", () => {
  test("form validation and clear all functionality", async ({ page }) => {
    ...
    const submitBtn = page.getByRole("button", { name: "Submit for Diagnosis" });
    await expect(submitBtn).toBeDisabled();

    // Fill some simple fields
    await page.getByPlaceholder("e.g., 45").fill("45");
    await page.getByPlaceholder(/Chest pain, shortness of breath/).fill("Headache");
    
    // Submit should still be disabled because at least one of medical history, transcript, or labs is required.
    await expect(submitBtn).toBeDisabled();

    // Add medical history
    await page.getByPlaceholder(/Past diagnoses, medications/).fill("Some history");
    await expect(submitBtn).toBeEnabled();

    // Use Clear All
    await page.getByRole("button", { name: "Clear All" }).click();
    await expect(page.getByPlaceholder("e.g., 45")).toHaveValue("");
    await expect(page.getByPlaceholder(/Past diagnoses, medications/)).toHaveValue("");
    await expect(submitBtn).toBeDisabled();
  });
  ...
  test("browser back/forward navigation works after completing a flow", async ({ page }) => {
    // Start with the form
    await page.goto("/");
    await expect(page.getByRole("heading", { name: "New Case" })).toBeVisible();
    ...
    await page.getByPlaceholder(/Chest pain, shortness of breath/).fill("Headache");
    await page.getByPlaceholder(/Past diagnoses, medications/).fill("None significant");
    await page.getByPlaceholder(/Doctor-patient encounter/).fill("Reports headache for 2 days");
  ...
  test("specialist status updates during analysis", async ({ page }) => {
    ...
    await page.getByPlaceholder(/Chest pain, shortness of breath/).fill("Chest pain");
    await page.getByPlaceholder(/Past diagnoses, medications/).fill("Previous MI");
    await page.getByPlaceholder(/Doctor-patient encounter/).fill("Chest pain for 1 hour");
    await page.getByPlaceholder(/Blood panels, urinalysis/).fill("Troponin elevated");
